...
========================================================================
    ...
    - 즉, 중복된 값을 가진 레코드는 입력할 수 없습니다.
    
    실습 배경
    - 회원 테이블에 이메일(email) 열을 추가하고, 이 열에 UNIQUE 제약조건을 설정합니다.
    - 이메일 주소는 원래 사용자마다 다르므로, 일반적으로 이메일 중복되면 안됩니다.
========================================================================
*/
...
-- 2단계: member 테이블 생성
create table member(
	mem_id char(8) not null primary key,	-- 회원의 ID: 중복 불가 무조건 열에 값 저장
    mem_name varchar(10) not null,
    height tinyint unsigned null,
    email char(30) null unique				-- 이메일 열에 중복 저장 불가
);
-- 💡 POINT!
...
-- 두 번째 회원 -> 이메일 입력안함: UNIQUE 조건 위반은 아님
insert into member values('TWC', '트와이스', 167, null);

-- 세 번째 회원 -> 이메일이 'pink@gmail.com' 로 중복됨 -> UNIQUE 조건 위반 -> 오류 발생!
...
create table member(
	mem_id char(8) not null primary key,				-- 회원의 ID: 중복 불가 무조건 열에 값 저장
    mem_name varchar(10) not null,
    height tinyint unsigned null check(height >= 100),	-- height 열에는 100이상의 키만 저장
    phone char(3) null
);
...
-- 4단계 phone 열에도 check 제약조건 설정을 추가할 수 있음(특정 지역번호만 허용)
alter table member
add constraint
check(phone IN('02', '031', '032', 054, 055, 061));
-- >>> phone 열에는 반드시 IN 절 안에 작성한 데이터들만 저장할 수 있다.
...
-- [성공] phone 열에 null 이 설정되어 있어 빈 값 허용
insert into member(mem_id, mem_name, height, phone)values('BLK', '블랙핑크', 163, null );

-- [성공] phone 열에 추가되는 값 '02' 이므로 허용함 -> check 제약조건대로 열에 추가
insert into member values('OML', '어린친구', 168, '02');

-- [실패] phone 열에 추가될 값이 '010' 이므로 허용되지 않음 -> check 제약조건 위반
...
-- 2단계: member 테이블 새로 생성(default 디폴트 제약조건을 height 열에 160으로 설정)
create table member(
	mem_id char(8) not null primary key,
    mem_name varchar(10) not null,
	height tinyint unsigned null default 160,	-- 입력이 없으면 기본 값 160이 자동으로 insert 추가됨
    phone1 char(3) null 
);
...
/*
	4단계: 데이터 입력(추가) 실습
*/
-- > 첫 번째 insert: 열에 추가할 값 명시적으로 입력
-- > height = 161, phone1 = '054' 직접 입력 -> 기본 값 사용 안함

describe member;
insert into member values('RED', '레드벨벳', 161, '054');

-- > 두 번째 insert: height 과 phone1 열에 추가할 값을 생략하고 기본값 사용
-- height = default -> height 에 자동으로 160 입력됨
-- phone1 = default -> phone1 에 자동으로 '02' 입력됨
...
create table member(
	...
	email char(30)					-- 이메일도 열에 입력하지 않아도 됨.(NULL) 허용
);

-- 예제8. 다양한 입력 케이스
-- 1. 모든 값을 정상적으로 입력하는 경우
insert into member values('BLK', '블랙핑크', 163, 'pink@gmail.com');
...
insert into member values('APN', NULL, 160, 'apink@gmail.com');

-- 4. 기본키 제약조건이 설정된 mem_id 열 값 입력을 생략한 경우 -> 오류가 발생함
insert into member values(NULL, '아이유', 160, 'iu@gmail.com');
...
# 2. 실제 테이블인 member 에서 필요한 열(mem_id, mem_name, addr) 값만 골라서 조회해 오기
...
# 4. 방금 만든 뷰 v_member 를 이용해 member 테이블의 mem_id, mem_name, addr 열의 값만 조회 가능
-- ▶ 실제 member 테이블을 직접 조회하지 않고도, 필요한 열만 필터링해서 확인 가능
select * from v_member;

# 5. 뷰를 이용한 조건 조회
-- > v_member 뷰에서 주소(addr)가 '서울' 또는 '경기'인 회원만 조회
-- > 원본 테이블 member 에 직접 접근해서 조회하지 않고 쉽게 필터링 기능
...
-- > buy 테이블과 member 테입ㄹ의 mem_id 열 값을 기준으로 inner join
...
-- 2. 뷰(v_memberbuy)명을 이용해서 위 조인한 select 전체의 조회 결과 얻기

select * from v_memberbuy;

-- 3. 뷰에서 조건을 걸어도 조회 가능(ex. 블랙핑크가 구매한 상품 목록 조회)
select * from v_memberbuy
where mem_name = '블랙핑크';
...
-- 뷰 생성: buy 테이블과 member 테이블을 조인해서 새로운 뷰(v_viewtest1)를 생성
-- > select 절에서 열에 별칭을 붙이고 있음
-- > 별칭은(Alias)은 작은 따옴표 ' ' 큰 따옴표 " ", 또는 AS 키워드를 사용해서 지정 가능

CREATE VIEW v_viewtest1
AS
	SELECT 
		B.mem_id 'Member ID', 			-- > buy 테이블의 회원 ID 에 'Member ID' 별칭 부여
        M.mem_name AS 'Member Name',	-- > member 테이블의 이름에 'Member Name' 별칭 부여
        B.prod_name "Product Name",		-- > 구매한 상품명에도 "Product Name" 별칭 부여
        CONCAT(M.phone1, M.phone2) AS "Office Phone"	-- > 연락처를 결합해서 "Office Phone" 별칭 부여
		FROM buy B INNER JOIN member M
		ON B.mem_id = M.mem_id;

-- -> v_viewtest1 뷰에서 중복 없이 (Member ID, Member Name) 열 값만 조회하는 쿼리(SQL 문)
...
as 
	SELECT 
		B.mem_id AS '회원 아이디', 		-- > buy 테이블의 회원 ID 에 'Member ID' 별칭 부여
        M.mem_name AS '회원 이름',			-- > member 테이블의 이름에 'Member Name' 별칭 부여
        B.prod_name AS "제품 이름",		-- > 구매한 상품명에도 "Product Name" 별칭 부여
...
from v_viewtest1;	-- 뷰를 마치 테이블처럼 사용하여 간단하게 조회가능
/*
조회결과
		회원아이디 회원이름
		APN		에이핑크
